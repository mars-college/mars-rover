...
Notes: There seem to be numerous errors and omissions in the instructions as provided. This guide documents my process so that others may avoid many wasted hours of effort.
...
## Enable the Hardware (only necessary for testing)
...
1. Check if the wwan0 interface is present: `$ ifconfig -a`
1. Enable the wwan0 interface: `$ sudo ifconfig wwan0 up`
1. `sudo minicom -D /dev/ttyUSB2` and run:
...
NOTE: This does not successfully automate all steps of the initialization process. I still need to figure out how to automate sending AT commands to the SIMCOM7600G-H with the `wwan_preup.sh` script. As it stands, you still have to open `minicom -D /dev/ttyUSB2` and run:
...
There are scripts included in this repo that allow you to install 4G connectivity at boot using `systemd` service files and a prestart script to automate the steps in the "Setup Network Interface `wwan0`" section above.

It's recommended that you clone the repo locally on the Jetson Nano.

1. `$ cd <path to mars rover repo>/connectivity`
1. `$ chmod +x wwan_preup.sh install.sh uninstall.sh`
