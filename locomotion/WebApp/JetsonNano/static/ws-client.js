$(document).ready(function(){
		...
		var ws = null;
		var ws_interval = 50; //sets the update interval in ms
		...
		ws.onopen = function(evt) {
			...
			//defaults to using the GUI joystick controller
			joyUpdate =  setInterval(sendJoy, ws_interval);
			};
		...
		function gamepadHandler(event, connecting) {
			...
			if (connecting) {
				...
				clearInterval(joyUpdate);
				gamepadUpdate = setInterval( sendGamepad, ws_interval);
			} else {
