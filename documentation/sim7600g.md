...
Notes: There seem to be numerous errors and omissions in the instructions as provided. So far, I've not been able to reach the device using `minicom`, but have somehow managed to get the 4G connection working. This guide documents my process so that others may avoid many wasted hours of effort.
...
At this point, the instructions provided by Waveshare call for using `minicom`, but doesn't provide any hint that it needs to be setup. Instructions for setup can be found [here](https://wiki.emacinc.com/wiki/Getting_Started_With_Minicom) and are summarized below.
...
1. Modify the Makefile: `$ nano Makefile`
...
1. `echo 'AT$QCRMCALL=1,1' > /dev/ttyTHS1`
1. Allocate IP: `$ sudo udhcpc -i wwan0`
